...
class Confirm extends \RestController
{
    ...
    private function pay($data)
    {
        ...
        if (isset($this->session->data['old_session_id'])){
            ...
            $this->db->query("REPLACE INTO `" . DB_PREFIX . "session` SET session_id = '" . $this->db->escape($this->session->data['old_session_id']) . "', `data` = '" . $this->db->escape(json_encode($this->session->data)) . "', expire = '" . $this->db->escape(date('Y-m-d H:i:s', time() + $expire)) . "'");
        }
