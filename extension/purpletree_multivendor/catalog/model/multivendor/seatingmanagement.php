...
class Seatingmanagement extends \Opencart\System\Engine\Model {
	public function addSeatingmanagement(int $table_id, array $data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "seatingmanagement SET table_no = '" . $this->db->escape($data['table_no']) . "', seat_capacity = '" . (int)$data['seat_capacity'] . "', vendor_store_id = '" . (int)$data['vendor_store_id'] . "',location = '" . (int)$data['location'] . "', status = '" . (bool)(isset($data['status']) ? $data['status'] : 0) . "', vendor_id = '".$this->customer->getId()."', date_modified = NOW(), date_added = NOW()");
	...
	public function getSeatingManagements($data = array()) {
		$sql = "SELECT * FROM ". DB_PREFIX ."seatingmanagement WHERE vendor_id = ".$this->customer->getId();		
